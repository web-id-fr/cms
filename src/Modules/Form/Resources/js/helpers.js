...
export function formErrorPopin(error, form) {
    ...
    //scroll to the first error
    let element = form.closest(".popin").find(".field-error span").first();
    $(".popin").animate( { scrollTop: $(element).offset().top - 400 }, 500 );
}
...
export function formSuccessPopin(form) {
    form.closest(".popin").find(".field-error span").remove();
    form.closest(".popin").find(".field-error br").remove();
    let $field_success = form.closest(".popin").find('.alert-success');
    $field_success.show();
    $field_success.delay(9000).fadeOut("slow");
}
